/**
 * Google Apps Script for Search Visibility Ranking Tracker - Palm Beaches Office
 *
 * Instructions:
...
/**
 * Sheet configuration - Palm Beaches specific
 */
const SHEET_NAMES = {
  RANK_MONITOR: 'palm-beaches',  // Changed from 'rankmonitor' to 'palm-beaches'
  SUBMIT_REQUESTS: 'pb_submit_requests',    // Palm Beaches specific audit tabs
  RESULTS_DUMP: 'pb_results_dump'
};

/**
 * Base column configuration for Palm Beaches sheet
 * Updated for palm-beaches tab with Population and Income columns
 */
const BASE_COLUMNS = {
  OFFICE: 0,       // Column A
  TARGETS: 1,      // Column B
...
function onOpen() {
  ...
  ui.createMenu('🌴 Palm Beaches Ranking Tracker')  // Changed menu title
...
function submitRankingJobs() {
  try {
    ...
    const response = ui.alert(
      'Submit Ranking Jobs - Palm Beaches',
      'This will submit ranking check jobs to DataForSEO for Palm Beaches. Continue?',
      ui.ButtonSet.YES_NO
    );

    if (response !== ui.Button.YES) return;

    ui.alert('⏳ Processing...', 'Reading Palm Beaches sheet data and submitting jobs. Please wait.', ui.ButtonSet.OK);
    ...
    ui.alert(
      ...
      `${jobCount} Palm Beaches ranking jobs submitted to DataForSEO.\\n\\nWait 2-5 minutes, then click "Get Results".`,
      ui.ButtonSet.OK
    );

  } catch (error) {
    SpreadsheetApp.getUi().alert('❌ Error', `Failed to submit Palm Beaches jobs: ${error.message}`, SpreadsheetApp.getUi().ButtonSet.OK);
...
function getRankingResults() {
  try {
    ...
    // Show confirmation dialog
    const response = ui.alert(
      'Get Ranking Results - Palm Beaches',
      'This will fetch results from DataForSEO and update the Palm Beaches sheet. Continue?',
      ui.ButtonSet.YES_NO
    );
    ...
    ui.alert(
      ...
      `Palm Beaches ranking data has been updated in the sheet.\\n\\nCheck the latest column for new rankings.`,
      ui.ButtonSet.OK
    );

  } catch (error) {
    SpreadsheetApp.getUi().alert('❌ Error', `Failed to get Palm Beaches results: ${error.message}`, SpreadsheetApp.getUi().ButtonSet.OK);
...
function dailyRankingCheck() {
  try {
    console.log('🤖 Starting automated Palm Beaches daily ranking check...');
    ...
    console.log('✅ Automated Palm Beaches daily ranking check completed successfully!');

  } catch (error) {
    console.error('❌ Automated Palm Beaches daily ranking check failed:', error);
    ...
    try {
      MailApp.sendEmail({
        ...
        subject: '❌ Palm Beaches Daily Ranking Check Failed',
        body: `The automated Palm Beaches daily ranking check failed with error: ${error.message}\n\nPlease check the Google Apps Script logs for more details.`
      });
    } catch (emailError) {
...
function submitRankingJobsAutomated() {
  console.log('📤 Starting automated Palm Beaches job submission...');
  ...
  // Log success
  const jobCount = Object.values(taskResults).flat().length;
  console.log(`✅ Automated Palm Beaches job submission completed: ${jobCount} jobs submitted`);
...
function getRankingResultsAutomated() {
  console.log('📥 Starting automated Palm Beaches results retrieval...');

  // Phase 1: Read task IDs from temporary storage
  const taskIds = getStoredTaskIds();

  if (taskIds.length === 0) {
    console.log('⚠️ No task IDs found for automated Palm Beaches results retrieval');
    return;
  }
  ...
  console.log('✅ Automated Palm Beaches results retrieval completed successfully!');
...
/**
 * Gets all data from the Palm Beaches sheet
 */
...
/**
 * Gets or creates the submit requests sheet for Palm Beaches
 */
...
/**
 * Gets or creates the results dump sheet for Palm Beaches
 */
...
function logSubmitRequest(primeUrl, requestData) {
  try {
    ...
    console.log(`📤 Palm Beaches submit request logged for: ${primeUrl}`);

  } catch (error) {
    console.error('Failed to log Palm Beaches submit request:', error);
  }
}
...
function logResultsDump(primeUrl, rawResponse) {
  try {
    ...
    sheet.appendRow(rowData);
    console.log(`📥 Palm Beaches results dump logged for: ${primeUrl}`);

  } catch (error) {
    console.error('Failed to log Palm Beaches results dump:', error);
  }
}

/**
 * Converts Google Sheets data to preflight structure
 * (Adapted from buildPreflight.js for Palm Beaches)
 */
    ...
    for (const item of items) {
      ...
      for (const keyword of keywords) {
        try {
          ...
          console.log(`✅ Palm Beaches job submitted successfully for ${prime_url}: ${taskId}`);
          ...
          console.error(`Error submitting Palm Beaches job for keyword "${keyword}":`, error);

          // Log error to audit
          console.error(`❌ Failed to submit Palm Beaches job for ${prime_url}: ${error.message}`);
...
function fetchResultsFromDataForSEO(taskIds) {
  ...
  for (const task of taskIds) {
    try {
      ...
      console.log(`📥 Fetching Palm Beaches results for ${task.prime_url}: ${task.taskId}`);
      ...
      if (taskResult.result && taskResult.result.length > 0) {
        ...
        console.log(`✅ Palm Beaches results fetched for ${task.prime_url}: Found ${rawSerpItems.length} items, Prime URL rank: ${rankings.length > 0 ? rankings[0].rank : 'Not found'}`);
        ...
        console.log(`⚠️ No Palm Beaches results found for ${task.prime_url}: ${task.taskId}`);
      ...
      console.error(`Error fetching Palm Beaches results for task ${task.taskId}:`, error);

      // Log error to audit
      console.error(`❌ Failed to fetch Palm Beaches results for ${task.prime_url}: ${error.message}`);
...
/**
 * Writes results back to the Palm Beaches sheet
 */
...
function checkJobStatus() {
  ...
  if (taskIds.length === 0) {
    SpreadsheetApp.getUi().alert('No Jobs Found', 'No active Palm Beaches jobs to check.', SpreadsheetApp.getUi().ButtonSet.OK);
    return;
  }

  SpreadsheetApp.getUi().alert(
    'Job Status',
    `Found ${taskIds.length} active Palm Beaches jobs.\\n\\nIf you submitted jobs 2-5 minutes ago, they should be ready.\\n\\nClick "Get Results" to fetch them.`,
...
function viewSubmitRequests() {
  try {
    ...
    SpreadsheetApp.getUi().alert(
      '📤 Submit Requests Log',
      `Palm Beaches submit requests log opened! This sheet shows all requests sent to DataForSEO.\\n\\nColumns:\\n- Prime URL: The specific URL being tracked\\n- Request Data: The complete request payload`,
      SpreadsheetApp.getUi().ButtonSet.OK
    );

  } catch (error) {
    SpreadsheetApp.getUi().alert('❌ Error', `Failed to open Palm Beaches submit requests: ${error.message}`, SpreadsheetApp.getUi().ButtonSet.OK);
  }
}
...
function viewResultsDump() {
  try {
    ...
    SpreadsheetApp.getUi().alert(
      '📥 Results Dump',
      `Palm Beaches results dump opened! This sheet shows all raw responses from DataForSEO.\\n\\nColumns:\\n- Prime URL: The specific URL being tracked\\n- Raw DataForSEO Response: Complete response including all SERP data`,
      SpreadsheetApp.getUi().ButtonSet.OK
    );

  } catch (error) {
    SpreadsheetApp.getUi().alert('❌ Error', `Failed to open Palm Beaches results dump: ${error.message}`, SpreadsheetApp.getUi().ButtonSet.OK);
  }
}

/**
 * Clears task data from Palm Beaches sheet and temporary storage
 */
function clearTaskData() {
  const response = SpreadsheetApp.getUi().alert(
    'Clear Task Data',
    'This will clear all ranking data from the Palm Beaches sheet and stored task IDs. Continue?',
  ...
  SpreadsheetApp.getUi().alert('✅ Cleared', 'Palm Beaches task data and stored task IDs have been cleared.', SpreadsheetApp.getUi().ButtonSet.OK);
}
...
function testDataForSEOConnection() {
  try {
    ...
    if (response.getResponseCode() === 200) {
      SpreadsheetApp.getUi().alert('✅ Connection Successful', 'DataForSEO API connection is working for Palm Beaches!', SpreadsheetApp.getUi().ButtonSet.OK);
    } else {
...
function storeTaskIds(taskResults) {
  ...
  // Store in Script Properties with timestamp (use unique key for Palm Beaches)
  const timestamp = new Date().getTime();
  PropertiesService.getScriptProperties().setProperty('palmBeachesTaskIds', JSON.stringify(taskData));
  PropertiesService.getScriptProperties().setProperty('palmBeachesTaskIds_timestamp', timestamp.toString());

  console.log(`Stored ${taskData.length} Palm Beaches task IDs temporarily`);
}
...
function getStoredTaskIds() {
  try {
    const taskData = PropertiesService.getScriptProperties().getProperty('palmBeachesTaskIds');
    const timestamp = PropertiesService.getScriptProperties().getProperty('palmBeachesTaskIds_timestamp');

    if (!taskData) {
      console.log('No stored Palm Beaches task IDs found');
      return [];
    }
    ...
    if (currentTime - storedTime > maxAge) {
      console.log('Stored Palm Beaches task IDs are too old, clearing them');
      clearStoredTaskIds();
      return [];
    }

    const parsed = JSON.parse(taskData);
    console.log(`Retrieved ${parsed.length} stored Palm Beaches task IDs`);
    return parsed;

  } catch (error) {
    console.error('Error retrieving stored Palm Beaches task IDs:', error);
    return [];
  }
}
...
function clearStoredTaskIds() {
  PropertiesService.getScriptProperties().deleteProperty('palmBeachesTaskIds');
  PropertiesService.getScriptProperties().deleteProperty('palmBeachesTaskIds_timestamp');
  console.log('Cleared stored Palm Beaches task IDs');
}
...
function setupDailyAutomation() {
  try {
    ...
    ui.alert(
      '📋 Setup Instructions',
      'To complete the Palm Beaches setup:\\n\\n1. Go to Apps Script (script.google.com)\\n2. Open your project\\n3. Click "Triggers" (clock icon on left)\\n4. Click "+ Add Trigger"\\n5. Choose:\\n   - Function: dailyRankingCheck\\n   - Event source: Time-driven\\n   - Type: Day timer\\n   - Time: Pick your preferred time\\n6. Click "Save"\\n\\nRecommended time: 9:00 AM (after business hours start)',
      ui.ButtonSet.OK
    );

    // Store automation preference
    PropertiesService.getScriptProperties().setProperty('palmBeachesAutomation_enabled', 'true');

  } catch (error) {
    SpreadsheetApp.getUi().alert('❌ Error', `Failed to setup Palm Beaches automation: ${error.message}`, SpreadsheetApp.getUi().ButtonSet.OK);
    console.error('Setup Palm Beaches automation error:', error);
  }
}
...
function disableAutomation() {
  try {
    ...
    const response = ui.alert(
      '🔕 Disable Palm Beaches Automation',
      'This will remove the Palm Beaches daily automation trigger. You can still run rankings manually.\\n\\nContinue?',
      ui.ButtonSet.YES_NO
    );
    ...
    PropertiesService.getScriptProperties().deleteProperty('palmBeachesAutomation_enabled');

    if (removedCount > 0) {
      ui.alert(
        '✅ Automation Disabled',
        `Removed ${removedCount} Palm Beaches automation trigger(s).\\n\\nDaily automation is now disabled. You can still run rankings manually using the menu.`,
        ui.ButtonSet.OK
      );
    } else {
      ui.alert(
        'ℹ️ No Automation Found',
        'No active Palm Beaches automation triggers were found to remove.',
        ui.ButtonSet.OK
      );
    }

  } catch (error) {
    SpreadsheetApp.getUi().alert('❌ Error', `Failed to disable Palm Beaches automation: ${error.message}`, SpreadsheetApp.getUi().ButtonSet.OK);
    console.error('Disable Palm Beaches automation error:', error);
  }
}
...
function testDailyRankingCheck() {
  try {
    ...
    const response = ui.alert(
      '🧪 Test Palm Beaches Daily Check',
      'This will run the complete automated Palm Beaches daily ranking check process.\\n\\nThis includes:\\n- Submit jobs\\n- Wait 5 minutes\\n- Get results\\n\\nThis may take 6-7 minutes total. Continue?',
      ui.ButtonSet.YES_NO
    );

    if (response !== ui.Button.YES) return;

    ui.alert('⏳ Starting Test...', 'Running automated Palm Beaches daily check. This will take about 6-7 minutes.\\n\\nYou can monitor progress in the Apps Script logs.', ui.ButtonSet.OK);
    ...
    ui.alert(
      '✅ Test Completed!',
      'Palm Beaches daily ranking check test completed successfully!\\n\\nCheck your sheet for updated rankings.',
      ui.ButtonSet.OK
    );

  } catch (error) {
    SpreadsheetApp.getUi().alert('❌ Test Failed', `Palm Beaches daily check test failed: ${error.message}\\n\\nCheck the Apps Script logs for more details.`, SpreadsheetApp.getUi().ButtonSet.OK);
    console.error('Test Palm Beaches daily check error:', error);
  }
}
...
/**
 * Sheet configuration - San Antonio Desktop specific
 */
const SHEET_NAMES = {
  RANK_MONITOR: 'san-antonio',  // Use same sheet as mobile version
  SUBMIT_REQUESTS: 'sa_desktop_submit_requests',    // San Antonio Desktop audit tabs
  RESULTS_DUMP: 'sa_desktop_results_dump'
};

/**
 * Base column configuration for San Antonio Desktop sheet
...
function onOpen() {
  ...
  ui.createMenu('🖥️ San Antonio Desktop Ranking Tracker')  // Desktop themed menu
...
function submitRankingJobs() {
  try {
    ...
    const response = ui.alert(
      'Submit Ranking Jobs - San Antonio Desktop',
      'This will submit ranking check jobs to DataForSEO for San Antonio (Desktop). Continue?',
      ui.ButtonSet.YES_NO
    );

    if (response !== ui.Button.YES) return;

    ui.alert('⏳ Processing...', 'Reading San Antonio Desktop sheet data and submitting jobs. Please wait.', ui.ButtonSet.OK);
    ...
    ui.alert(
      ...
      `${jobCount} San Antonio Desktop ranking jobs submitted to DataForSEO.\\n\\nWait 2-5 minutes, then click "Get Results".`,
      ui.ButtonSet.OK
    );

  } catch (error) {
    SpreadsheetApp.getUi().alert('❌ Error', `Failed to submit San Antonio Desktop jobs: ${error.message}`, SpreadsheetApp.getUi().ButtonSet.OK);
...
function getRankingResults() {
  try {
    ...
    // Show confirmation dialog
    const response = ui.alert(
      'Get Ranking Results - San Antonio Desktop',
      'This will fetch results from DataForSEO and update the San Antonio Desktop sheet. Continue?',
      ui.ButtonSet.YES_NO
    );
    ...
    ui.alert(
      ...
      `San Antonio Desktop ranking data has been updated in the sheet.\\n\\nCheck the latest column for new rankings.`,
      ui.ButtonSet.OK
    );

  } catch (error) {
    SpreadsheetApp.getUi().alert('❌ Error', `Failed to get San Antonio Desktop results: ${error.message}`, SpreadsheetApp.getUi().ButtonSet.OK);
...
/**
 * Gets all data from the San Antonio Desktop sheet
 */
...
/**
 * Gets the San Antonio Desktop sheet
 */
...
/**
 * Gets or creates the submit requests sheet for San Antonio Desktop
 */
function getSubmitRequestsSheet() {
  ...
  if (!sheet) {
    ...
    headerRange.setBackground('#e8f5e8'); // Light green for San Antonio desktop theme
...
/**
 * Gets or creates the results dump sheet for San Antonio Desktop
 */
